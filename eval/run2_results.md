# Kết quả Eval Lượt 2 — Prompt v2 (nhánh thử nghiệm của Lương)

> **Ghi chú khi merge CP4:** Tài liệu này ghi nhận một phiên bản `codebase/core/prompt.py` khác (viết lại toàn bộ system prompt) do một thành viên thử nghiệm độc lập song song với bản vá tăng dần đang được dùng chính thức trong repo. Khi merge, nhóm quyết định **giữ bản `prompt.py` đang chạy chính thức** (không phải bản mô tả trong file này), nên các số liệu latency/model dưới đây **không phản ánh code hiện tại**. Số liệu chính thức cho Quality Bar nằm ở `spec.md` §7.4 và `eval/run_results.md`. File này được giữ lại làm tài liệu tham khảo cho hướng tiếp cận thay thế (đặc biệt là ý tưởng post-processing ép cứng `confidence` theo `author_role` trong `codebase/core/extractor.py`, đã được giữ lại và merge vào code chính thức).

## Kết quả chính (đo trên phiên bản prompt không còn dùng)
